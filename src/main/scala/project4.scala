  ...
  def main(args: Array[String]): Unit = {
    ...
    if(isServer){
      ...
      for(i <- 0 to numberOfLBs - 1) {
        var server = system.actorOf(Server.props("twitter-client", clientProps.getString("akka.remote.netty.tcp.hostname"), clientProps.getString("akka.remote.netty.tcp.port")), "server" + i)
        if(i == 0) {
